...
export default function RatesView(props) {
  ...
  return (
    <Grid container spacing={3} sx={{ placeContent: "center", py: 2 }}>
      <Grid item xs={6}>
        <Typography variant="subtitle1" color="primary" className="font-bold font-gray"> {props.literals.rates}</Typography>
      </Grid>
      <Grid item xs={6} align="right">
        <Button
          ...
          disabled={helpers.abilityByModuleKey('rates').can_add == false}
        >
          {props.literals.add} +
        </Button>
      </Grid>
                                ...
                                <Table size="small">
                                  ...
                                  {props.rateDetail.map(x => {
                                    return (
                                      <TableBody>
                                        <TableRow>
                                          <TableCell>{x.rate_name} {(x.special_rate === true) ? '(' + props.literals.special_rate + ')' : '(' + props.literals.regular_rate + ')'}</TableCell>
                                          <TableCell>{days.map(el => {
                                            return (x[el.name] === true ? el.label + ', ' : '')
                                          })}</TableCell>
                                          <TableCell>{(x.special_rate === true) ?
                                            moment.utc(x.start_date).format('ll hh:mm a')
                                            + ' - ' +
                                            moment.utc(x.end_date).format('ll hh:mm a') : 'Always'}
                                          </TableCell>
                                          ...
                                          <TableCell >
                                            <Button
                                              ...
                                              disabled={helpers.abilityByModuleKey('rates').can_delete == false}
                                            ...
                                            <Button
                                              type="button"
                                              disabled={helpers.abilityByModuleKey('rates').can_edit == false}
