...
export default function VirtualMeterAdsView(props) {
    ...
    return (
      <Grid container spacing={3} sx={{placeContent: "center", py: 2}}>
        <Grid item xs={6}>
          <Typography variant="subtitle1" color="primary" className="font-bold font-gray">{props.literals.virtual_meter_ads}</Typography>
        </Grid>
        ...
        <Grid item xs={12}>
          <Paper elevation={0} sx={{ width: '100%', overflow: 'hidden' }}>
                  ...
                  <Table aria-label="collapsible table" size="small">
                      <TableHead>
                          <TableRow>
                              ...
                              <StyledTableCell>{props.literals.start_date}</StyledTableCell>
                              <StyledTableCell>{props.literals.end_date}</StyledTableCell>
                              <StyledTableCell>{props.literals.action}</StyledTableCell>
                          </TableRow>
                      </TableHead>
                      <TableBody>
                          {props.virtualMeterAds
                              ...
                              return (
                                  <>
                                      <TableRow key={row.zone}>
                                          ...
                                          <TableCell>{moment(row.start_date).tz(row?.zone?.city_id?.time_zone || 'America/New_York' ).format('ll hh:mm a')}</TableCell>
                                          <TableCell>{moment(row.end_date).tz(row?.zone?.city_id?.time_zone || 'America/New_York' ).format('ll hh:mm a')}</TableCell>
                                          <TableCell >
                                              <Button 
                                                  type="button" 
              ...
              <TablePagination
                  rowsPerPageOptions={[10, 25, 100, { label: 'All', value: props.virtualMeterAds.length }]}
                  component="div"
                  count={props.virtualMeterAds.length}
